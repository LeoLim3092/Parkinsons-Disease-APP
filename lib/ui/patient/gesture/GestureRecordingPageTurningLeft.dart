...
import 'package:pd_app/ui/patient/gesture/GestureRecordingPageTurningRight.dart';

import 'package:pd_app/utils/UploadUtil.dart';
import 'package:pd_app/utils/TimeUtil.dart';


class GestureRecordingPageTurningLeft extends StatefulWidget {
  List<CameraDescription> cameras;
  Patient patient;

  /// Default Constructor
  GestureRecordingPageTurningLeft({Key? key, required this.cameras, required this.patient}) : super(key: key);

  @override
  State<GestureRecordingPageTurningLeft> createState() {
    return _GestureRecordingPageTurningLeftState();
  }
}
...
class _GestureRecordingPageTurningLeftState extends State<GestureRecordingPageTurningLeft> with WidgetsBindingObserver, TickerProviderStateMixin {
    ...
    stopVideoRecording().then((XFile? file) {
      ...
      if (file != null) {
        showUploadDialog(
          ...
          onSuccessNavigation: () => gotoGesturePageTurningRight(widget.patient),
          dialogTitle: "上傳",
          dialogContent: "請問您是否要上傳此左手旋轉錄影？",
          cancelText: "取消",
          uploadText: "上傳",
        );
      }
  ...
  void gotoGesturePageTurningRight(Patient patient) async {
    final cameras = await availableCameras();
    Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (context) => GestureRecordingPageTurningRight(
          cameras: cameras,
          patient: patient,
        )));
  }
