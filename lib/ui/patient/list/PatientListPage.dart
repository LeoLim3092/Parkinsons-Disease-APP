...
//               decoration: const InputDecoration(border: UnderlineInputBorder(), labelText: '請輸入名字或流水號搜尋'),
...
//
//         ];
//         children.addAll(state.patients.where((patient) => (patient.name ?? "").contains(searchController.text)).toList().map((patient) => getPatientComponent(patient)));
//
//         return Scaffold(
//           appBar: AppBar(title: const Text("選擇病人")),
//           body: RefreshIndicator(
//             onRefresh: () async { context.read<PatientListCubit>().fetch();
//             setState(() {
//
//             List<Widget> children = [
//
