...
class PaintSpiralLeftHandPage extends StatefulWidget {
  final Patient patient;
  const PaintSpiralLeftHandPage({Key? key, required this.patient})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => _PaintSpiralLeftHandPageState();
}

class _PaintSpiralLeftHandPageState extends State<PaintSpiralLeftHandPage> {
  final _controller = HandwrittenSignatureController();
  Uint8List? _savedImage;
  // List<int>? _savedImage;
  ...
  void showPromptDialog() {
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
              title: const Text("提示", style: TextStyle(fontSize: 28)),
              content: const Text("請用左手依紅線軌跡畫同心圓",
                  style: TextStyle(fontSize: 28)),
              actions: <Widget>[
                TextButton(
                  child: const Text("確認", style: TextStyle(fontSize: 28)),
                  onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("同心圓繪圖-左手",
          style: TextStyle(fontSize: 28))),
      body: Column(
        children: [
          Expanded(
                ...
                TextButton(
                  ...
                  onPressed: () async {
                    ...
                    Navigator.of(context).pushReplacement(MaterialPageRoute(builder:
                        (context) => QuestionPage(patient: widget.patient)));
                  },
                ),
              ]);
        });
...
//     ..isAntiAlias = true;
//
//   final _startEndPointPaint = Paint()
//     ..color = Colors.blue
...
//     for (Path path in circleList) {
//       canvas.drawPath(path, _circlePaint);
//       drawStartEndPoints(canvas, path);
//     }
//     for (Path? path in pathList) {
//       _drawLine(canvas, path);
//     }
...
//   }
//
//   void drawStartEndPoints(Canvas canvas, Path path) {
//     // Extract points from path
//     var metrics = path.computeMetrics(forceClosed: false);
//     var start = metrics.firstWhere((m) => m.length > 0);
//     var end = metrics.lastWhere((m) => m.length > 0);
//
//     var startTangent = start.getTangentForOffset(0.0);
//     var endTangent = end.getTangentForOffset(end.length);
//
//     if (startTangent != null && endTangent != null) {
//       var startPoint = startTangent.position;
//       var endPoint = endTangent.position;
//
//       canvas.drawCircle(startPoint, 5, _startEndPointPaint);
//       canvas.drawCircle(endPoint, 5, _startEndPointPaint);
//     }
//   }
//
//   void _drawLine(Canvas canvas, Path? path) {
