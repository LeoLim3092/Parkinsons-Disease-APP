...
import 'package:pd_app/ui/patient/paint/PaintSpiralLeftHandPage.dart';

class PaintSpiralRightHandPage extends StatefulWidget {
  final Patient patient;
  const PaintSpiralRightHandPage({Key? key, required this.patient})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => _PaintSpiralRightHandPageState();
}

class _PaintSpiralRightHandPageState extends State<PaintSpiralRightHandPage> {
  ...
  void initState() {
    super.initState();
    Future(() => showHandednessnDialog());
  }

  void showHandednessnDialog() {
    showDialog(
        context: context,
        barrierDismissible: false,
        builder: (context) {
          return AlertDialog(
              title: const Text("諮詢", style: TextStyle(fontSize: 28)),
              content: const Text("請問您的慣用手是右手還是左手?",
                  style: TextStyle(fontSize: 28)),
              actions: <Widget>[
                TextButton(
                  child: const Text("左手", style: TextStyle(fontSize: 28)),
                  onPressed: () {
                    handedness = '1';
                ...
                TextButton(
                  child: const Text("右手", style: TextStyle(fontSize: 28)),
                  onPressed: () {
                    handedness = '0';
                    Navigator.of(context).pop();
                    showPromptDialog();
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
              ...
              content: const Text("請用右手依紅線軌跡畫同心圓",
                  style: TextStyle(fontSize: 28)),
              actions: <Widget>[
                TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("同心圓繪圖-右手",
          style: TextStyle(fontSize: 28))),
      body: Column(
        children: [
          Expanded(
